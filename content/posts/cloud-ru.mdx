...
2. Перейдите в раздел **Виртуальные машины** → **Создать ВМ**.
3. Выберите образ **Ubuntu 22.04 LTS**.
4. Укажите параметры: CPU, RAM, размер диска.
5. Подключите **внешний IP-адрес** (в Cloud.ru внешний IP стоит около **150 ₽/мес**).
6. Задайте имя пользователя (например, `ubuntu` или другое). Этот пользователь будет использоваться для SSH-подключения.
